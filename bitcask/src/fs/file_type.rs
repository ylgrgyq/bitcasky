...
impl FileType {
    pub fn get_path<P: AsRef<Path>>(&self, base_dir: P, file_id: Option<StorageId>) -> PathBuf {
    ...
    pub fn parse_file_id_from_file_name(&self, file_path: &Path) -> Option<StorageId> {
        ...
        .map(|file_id_str| file_id_str.parse::<StorageId>())
        .transpose()
        .unwrap_or(None)
    }
